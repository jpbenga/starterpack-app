...
export class AuthService {
  ...
  constructor(private auth: Auth) {
    ...
    this.isPremium$ = this.user$.pipe(
      switchMap(user => {
        if (user) {
          return getIdTokenResult(user, true).then(idTokenResult => { // true force le rafraîchissement
    ...
    // Déclenche la première vérification
    ...
    try {
        ...
        if (this.anonymousUserId && this.auth.currentUser && !this.auth.currentUser.isAnonymous) {
            ...
            this.anonymousUserId = null;
             ...
             // Si l'utilisateur n'était pas anonyme avant, on s'assure que l'état premium est vérifié
             this.forceClaimRefresh(); // Force la vérification des claims après connexion
        }
        return userCredential.user;
    } catch (error: any) {
    ...
    async signUpWithEmail(email: string, password: string): Promise<User | null> {
        ...
        try {
            if (currentUser && currentUser.isAnonymous) {
                ...
                this.anonymousUserId = null;
                this.forceClaimRefresh(); // Force la vérification des claims après liaison
                // Envoyer la vérification email après liaison si nécessaire
                ...
                this.anonymousUserId = null;
                this.forceClaimRefresh(); // Force la vérification des claims après inscription
                return userCredential.user;
            }
        } catch (error: any) {
  ...
  async signOutUser(): Promise<void> {
    try {
      ...
      this.anonymousUserId = null;
      this._isPremiumSubject.next(false); // Assure que le statut premium est réinitialisé
  ...
  // Nouvelle méthode pour forcer la revérification des claims
  async forceClaimRefresh(): Promise<boolean> {
    const user = this.auth.currentUser;
    if (user) {
      try {
        const idTokenResult = await getIdTokenResult(user, true); // true force le rafraîchissement
        const isPremium = idTokenResult?.claims['premium'] === true;
        this._isPremiumSubject.next(isPremium);
    ...
    // Si pas d'utilisateur, s'assurer que le statut est false
